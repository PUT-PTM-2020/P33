...
//Font array coding every ascii character
const short FONT[][8] = {
...
//Defined variables NUM_DIGITS = number of diodes in single matrix, cascaded = number of matrices
int NUM_DIGITS = 8;
short cascaded = 8;
...
//return a copy of orginal font array for use
short* value(char asciiCode) {
	short* font = malloc(8*sizeof(short));

	for(int i = 0 ; i < 8;i++) {
		font[i] = FONT[(uint8_t)asciiCode][i];
	}
	return font;
}
...
void max_init(SPI_HandleTypeDef* spi) {
	...
	SPI = spi;
	//buffer initializer
	 buffer = malloc(NUM_DIGITS*cascaded*sizeof(buffer));

	 //starting command
	 command(REG_SCANLIMIT, (uint8_t)0x7);
     command(REG_DECODEMODE, (uint8_t)0x0);
     command(REG_DISPLAYTEST,(uint8_t) 0x0);
     //command(REG_SHUTDOWN, 0x1);
     //setting brightness
     brightness( (uint8_t)0x3);
     //clear();
}

//command processing for sending
...
void clear() {
	for(int i = 0;i<cascaded; i++) {
		for(short j=0; j < NUM_DIGITS;j++) {
			setByte(i, (short) (j + REG_DIGIT0), (uint8_t)0x00);
		}
	}
...
void rotate_left(uint8_t redraw) {
	uint8_t t = buffer[NUM_DIGITS*cascaded-1];
	for(int i = NUM_DIGITS*cascaded -1; i > 0;i--) {
		buffer[i] = buffer[i-1];
	}
	buffer[0] = t;
...
void rotate_right(uint8_t redraw) {
	uint8_t t = buffer[0];
	for(int i = 0; i < NUM_DIGITS*cascaded -1;i++) {
		buffer[i] = buffer[i+1];
	}
	buffer[NUM_DIGITS*cascaded-1] = t;
...
void scroll_left(uint8_t redraw) {
	...
	buffer[NUM_DIGITS * cascaded - 1] = 0x0;
...
void letter(short deviceId, short asciiCode, uint8_t redraw) {
	short* values = value(asciiCode);

	short col = REG_DIGIT0;
	for (int i = 0;i<8;i++) {
		if (col > REG_DIGIT7) return;
		setByte(deviceId, col, (uint8_t) (values[i] & 0xff));
		col += 1;
	}
...
void showMessage(char* text,int size) {
	...
	uint8_t* src = malloc(NUM_DIGITS*size*sizeof(src));

	for(int i = 0;i< size;i++) {
		char c = text[i];
		short* values = value(c);
		for(int j = 0;j<8;j++) {
			src[i*NUM_DIGITS+j] = (uint8_t)(values[j] & 0xff);
		}
		free(values);
	}
	...
	for(int i = 0; i <NUM_DIGITS*size;i++ ) {
		...
		HAL_Delay(30);
		scroll_left(1);
		buffer[NUM_DIGITS*cascaded - 1] = src[i];
...
void flush() {
	int size = NUM_DIGITS*cascaded;
	uint8_t* buf = malloc(size*sizeof(uint8_t));
	uint8_t* removeBuf = buf;
	...
	for(uint16_t pos = 0;pos < NUM_DIGITS;pos++) {
		uint8_t* val = values(pos, buf);
		write(val,2*cascaded);
		free(val);
	}
	free(removeBuf);
	free(buf);
...
void brightness(uint8_t intense) {
	if(intense < 0 || intense > 15) return;

	command(REG_INTENSITY,intense);
}

void close() {
	clear();
	command(REG_SHUTDOWN,(uint8_t) 0x0);
}

void open() {
	command(REG_SHUTDOWN,(uint8_t) 0x1);
	clear();
}

//Writing function into SPI Bus
void write(uint8_t buf[],int size) {
	//DEFAULT CS PIN PA4
	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_4,GPIO_PIN_RESET);
	HAL_SPI_Transmit(SPI, (uint8_t*)buf, size, HAL_MAX_DELAY);
	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_4,GPIO_PIN_SET);
}

//calculate values for sending array
uint8_t* values(short position, uint8_t buf[]) {
	int len = 2*cascaded;
	uint8_t* ret = malloc(len*sizeof(uint8_t));

	for(int i = 0; i< cascaded;i++) {
		ret[2*i] = (uint8_t)((position+REG_DIGIT0) & 0xff);
...
//setting byte in exact place inside matrix array
void setByte(int deviceId, short position, uint8_t value) {
	int offset = deviceId * NUM_DIGITS + position - REG_DIGIT0;
	buffer[offset] = value;
}

//rotate single 8x8 matrix
...
//apply rottation for all cascaded martices
uint8_t* rotate(uint8_t buf[]) {
	uint8_t* result = malloc(NUM_DIGITS*cascaded*sizeof(uint8_t));
	for(int i = 0 ; i < NUM_DIGITS*cascaded; i+= NUM_DIGITS) {
		uint8_t* tile = malloc(NUM_DIGITS*sizeof(uint8_t));
		uint8_t* removeTile = tile;
		for(int j = 0;j< NUM_DIGITS;j++) {
			tile[j] = buf[i+j];
		}
		int k = orientation/90;
		for(int j = 0;j < k;j++) {
			tile = rotate88(tile);
		}
